...
import { VariantGenerator } from "./variants.js";
...
export class ReactComponentGenerator {
    ...
    async generate(input) {
        ...
        const code = this.generateComponentCode(componentName, jsxCode, propsDestructuring, propsInterface, context.exportStyle, context.includeTypescript, variants);
        // Get dependencies
    ...
    formatDesignTokens(tokens) {
        ...
        if (Object.keys(fontWeight).length > 0)
            extend.fontWeight = fontWeight;
        if (Object.keys(lineHeight).length > 0)
            extend.lineHeight = lineHeight;
     ...
     */
    generateComponentCode(componentName, jsxCode, propsDestructuring, propsInterface, exportStyle, includeTypescript, variants) {
        const lines = [];
        // Imports
