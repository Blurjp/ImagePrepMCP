...
export class ColorMapper {
    designTokens;
    constructor(designTokens = new Map()) {
     ...
     */
    mapStrokeWeight(weight) {
        // Tailwind's border width scale: 0, 1, 2, 4, 8
        const scale = [0, 1, 2, 4, 8];
        const closest = scale.reduce((prev, curr) => Math.abs(curr - weight) < Math.abs(prev - weight) ? curr : prev);
    ...
    mapBorderRadius(node) {
        ...
        const topLeftRadius = node.rectangleCornerRadii?.[0];
        ...
        const closest = scale.reduce((prev, curr) => Math.abs(curr - effectiveRadius) < Math.abs(prev - effectiveRadius) ? curr : prev);
