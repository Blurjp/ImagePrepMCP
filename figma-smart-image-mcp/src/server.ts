...
type ExportFigmaToReactArgs = z.infer<typeof ExportFigmaToReactInputSchema>;
...
class FigmaSmartImageServer {
  ...
  private authErrorResponse(detail?: string) {
      ...
      return {
        tools: [
          ...
          },
          {
            ...
            description:
              "Get all components and component sets from a Figma file. " +
          ...
          },
          {
            ...
            description:
              "Get detailed layout and styling information for a specific Figma node. " +
          ...
          },
          {
            ...
            description:
              "Get design variables (colors, spacing, typography) from a Figma file. " +
          ...
          },
          {
            ...
            description:
              "Export a Figma design as a React component with Tailwind CSS classes. " +
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      const runTool = async () => {
        ...
        if (toolName === "export_figma_to_react") {
          return await this.handleExportFigmaToReact(request.params.arguments as any);
        }
  ...
  private async handleGetFigmaComponents(args: GetFigmaComponentsArgs) {
    try {
  ...
  private async handleGetFigmaNodeDetails(args: GetFigmaNodeDetailsArgs) {
    try {
      // Extract session ID from metadata if available
      const sessionId = (args as any)._meta?.sessionId;
      const token = await this.getTokenForSession(sessionId || "");
  ...
  private async handleGetFigmaVariables(args: GetFigmaVariablesArgs) {
    try {
      // Extract session ID from metadata if available
      const sessionId = (args as any)._meta?.sessionId;
      const token = await this.getTokenForSession(sessionId || "");
      ...
      if (variablesList.length > 0) {
        ...
        // Group by type
        const byType: Record<string, typeof variablesList> = {};
...
`,
  ...
  private async handleExportFigmaToReact(args: ExportFigmaToReactArgs) {
    try {
      // Extract session ID from metadata if available
      const sessionId = (args as any)._meta?.sessionId;
      const token = await this.getTokenForSession(sessionId || "");
    ...
    const httpServer = createHttpServer(async (req, res) => {
      ...
      if (url.pathname === "/mcp") {
        ...
        if (useStreamableTransport) {
          ...
          if (req.method === "POST") {
            ...
            req.on("end", async () => {
              ...
              if (authSessionId && parsedBody) {
                const injectSession = (message: any) => {
              ...
              await streamableTransport.handleRequest(req as any, res as any, parsedBody);
            });
            return;
          }
          ...
          await streamableTransport.handleRequest(req as any, res as any);
          return;
        }
