...
import type { PropDefinition, BuildContext, ExtendedNodeDetails } from "../utils/types.js";
...
export interface VariantDefinition {
  ...
  variantStyles: Map<string, string>; // Maps "propertyName=value" to Tailwind classes
}
...
export class VariantGenerator {
   ...
   */
  extractVariants(
    componentSetNode: FigmaNodeDetails,
    getAllNodeDetails: (nodeId: string) => ExtendedNodeDetails | undefined
   ...
   */
  private extractVariantStyles(node: ExtendedNodeDetails): string[] {
  ...
  private getCommonStylesForVariant(
    matchingInstances: VariantInstance[],
    allInstances: VariantInstance[],
    ...
    // Get styles that appear in ALL matching instances
    const commonStyles = new Set<string>();
    ...
    const otherInstances = allInstances.filter(
      (inst) => inst.propertyValues[property.name] !== value
    );
    ...
    for (const style of commonStyles) {
      ...
      const appearsInOther = otherInstances.some((inst) =>
        inst.styles.includes(style)
      );
   ...
   */
  private parseVariantName(name: string): Record<string, string> | null {
  ...
  isVariantProp(propName: string, variants: VariantDefinition | null): boolean {
    ...
    return variants.properties.some(
      (p) => sanitizeIdentifier(p.name) === propName
    );
  }
