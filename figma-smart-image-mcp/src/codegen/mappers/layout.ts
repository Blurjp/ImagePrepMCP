...
export class LayoutMapper {
   ...
   */
  private mapPrimaryAxisAlign(node: FigmaNodeDetails): string {
    // Figma doesn't directly expose this in node details, infer from defaults
    // Default is MIN (start)
    const primaryAlign = (node as any).primaryAxisAlignMode || "MIN";
   ...
   */
  private mapCounterAxisAlign(node: FigmaNodeDetails): string {
    const counterAlign = (node as any).counterAxisAlignMode || "MIN";
   ...
   */
  private mapPadding(node: FigmaNodeDetails): string[] {
    const classes: string[] = [];
    ...
    // Individual sides
    if (pt > 0) classes.push(this.mapSpacing(pt, "pt"));
    if (pr > 0) classes.push(this.mapSpacing(pr, "pr"));
    if (pb > 0) classes.push(this.mapSpacing(pb, "pb"));
    if (pl > 0) classes.push(this.mapSpacing(pl, "pl"));
   ...
   */
  private mapSizingMode(node: FigmaNodeDetails): string[] {
    const classes: string[] = [];
