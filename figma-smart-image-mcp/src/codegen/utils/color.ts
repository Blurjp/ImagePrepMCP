 ...
 */
export function findClosestTailwindColor(hex: string): string | null {
  // Tailwind's default color palette (simplified)
  ...
  const targetColor = hexToRgba(hex);

  let minDistance = Infinity;
  let closestColor: { name: string; shade: number } | null = null;

  for (const [colorName, shades] of Object.entries(tailwindColors)) {
    for (const [shadeStr, shadeHex] of Object.entries(shades)) {
      const shade = parseInt(shadeStr, 10);
      const shadeColor = hexToRgba(shadeHex);
      const distance = colorDistance(targetColor, shadeColor);
  ...
  // Only return a match if the color is reasonably close
  if (minDistance < 0.15 && closestColor) {
    return `${closestColor.name}-${closestColor.shade}`;
  }
 ...
 */
export function isFillVisible(fill: FillStyle): boolean {
